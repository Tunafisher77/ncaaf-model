...
function runNcaafBestCardFridayCheck() {
  ...
  if (Utilities.formatDate(new Date(), zone, 'u') !== '5') return;
  var lock = LockService.getScriptLock();
  if (!lock.tryLock(30000)) return;
  try {
    var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(NCAAF_BEST_CARD_TAB);
    ...
    if (props.getProperty(NCAAF_BEST_CARD_SENT_WEEK) === key) return;
    var html = '<div style="font-family:Arial,sans-serif;font-size:14px;color:#111">';
    values.forEach(function(row) {
      var a = row[0] || '', b = row[1] || '';
      if (!a && !b) { html += '<br>'; return; }
      if (a === 'Weekly NCAAF Top 25 Best Card' || a === "LAST WEEK'S RESULTS" || a === "THIS WEEK'S BEST CARD") {
        html += '<h2 style="margin-top:24px">' + escapeNcaaf_(a) + '</h2>'; return;
      }
      if (a.indexOf('GAME ') === 0) { html += '<h3 style="margin-top:20px">' + escapeNcaaf_(a) + '</h3><p>' + escapeNcaaf_(b) + '</p>'; return; }
...
function installNcaafBestCardFridayTriggers() {
  ...
  [6, 7, 8, 9, 10, 11, 12].forEach(function(hour) {
    ScriptApp.newTrigger('runNcaafBestCardFridayCheck').timeBased().onWeekDay(ScriptApp.WeekDay.FRIDAY)
